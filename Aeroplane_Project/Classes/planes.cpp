...
void Planes::setCard(EventCustom* event) {
    ...
    can_touch = true;
    if (!cardInfoArray[2] && color != cardInfoArray[3]) {
        can_touch = false;
    }
}

// when a card is activated
void Planes::resetCard(EventCustom* event) {
    card = "none";
    round_left_of_card = 0;
    can_touch = false;
}
